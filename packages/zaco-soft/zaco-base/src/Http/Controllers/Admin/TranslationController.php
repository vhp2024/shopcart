...
class TranslationController extends Controller
{
    ...
    public function getIndex($group = null)
    {
        ...
        return view('zaco-base::translation-manager.index')
            ...
            ->with('editUrl', $group ? action('\ZacoSoft\ZacoBase\Http\Controllers\Admin\TranslationController@postEdit', [$group]) : null)
    ...
    public function postAddGroup(Request $request)
    {
        ...
        if ($group) {
            return redirect()->action('\ZacoSoft\ZacoBase\Http\Controllers\Admin\TranslationController@getView', $group);
        } else {
            return redirect()->back();
        }
    }
    ...
    public function postTranslateMissing(Request $request)
    {
        ...
        if ($request->has('with-translations') && $request->has('base-locale') && in_array($request->input('base-locale'), $locales) && $request->has('file') && in_array($newLocale, $locales)) {
            ...
            foreach ($base_strings as $base_string) {
                ...
                app()->call(
                    'ZacoSoft\ZacoBase\Controllers\Panel\TranslationController@postEdit',
                    [
                        'group' => $group,
                    ]
                );
